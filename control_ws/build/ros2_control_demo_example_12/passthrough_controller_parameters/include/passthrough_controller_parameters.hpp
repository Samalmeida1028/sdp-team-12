...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    : ParamListener(parameters_interface, rclcpp::get_logger("passthrough_controller"), prefix) {
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "interfaces")) {
          ...
          descriptor.description = "Names of the interfaces to be forwarded";
      ...
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'interfaces': {}", validation_result.error()));
      }
      if(auto validation_result = unique<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'interfaces': {}", validation_result.error()));
    ...
    private:
      ...
      rclcpp::Logger logger_ = rclcpp::get_logger("passthrough_controller");
