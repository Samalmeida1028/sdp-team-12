...
class DFR0601:
    def __init__(
        self, PWM1_pin, INA1_pin, INB1_pin, PWM2_pin, INA2_pin, INB2_pin, freq
    ):
        self.PWM1 = pwmio.PWMOut(
            PWM1_pin, duty_cycle=0, frequency=freq, variable_frequency=True
        )
        self.INA1 = digitalio.DigitalInOut(INA1_pin)
        self.INB1 = digitalio.DigitalInOut(INB1_pin)
        self.PWM2 = pwmio.PWMOut(
            PWM2_pin, duty_cycle=0, frequency=freq, variable_frequency=True
        )
...
class ChassisController:
    ...
    def set_bl_wheel(self, speed: int = 15000):
        ...
        self.motor_controller_1.PWM1.frequency = abs(speed + 2001)
        self.motor_controller_1.PWM1.duty_cycle = min(25000, max(3700, abs(speed)))
    ...
    def set_fl_wheel(self, speed: int = 15000):
        ...
        self.motor_controller_1.PWM2.frequency = abs(speed + 2001)
        self.motor_controller_1.PWM2.duty_cycle = min(25000, max(4000, abs(speed)))
    ...
    def set_fr_wheel(self, speed: int = 15000):
        ...
        self.motor_controller_2.PWM1.frequency = abs(speed + 2001)
        self.motor_controller_2.PWM1.duty_cycle = min(25000, max(4000, abs(speed)))
    ...
    def set_br_wheel(self, speed: int = 15000):
        ...
        self.motor_controller_2.PWM2.frequency = abs(speed + 2001)
        self.motor_controller_2.PWM2.duty_cycle = min(25000, max(4000, abs(speed)))
...
class Encoder:
    def __init__(self, pin1, pin2, time):
        self.current_time = time
        self.previous_time = self.current_time
        self.encoder = rotaryio.IncrementalEncoder(pin1, pin2, 1)
        self.previous_pos = 0.0

    def get_data(self, time):
        self.current_time = time
        # print((self.current_time-self.previous_time)/1E9)
        position = float(self.encoder.position/537.7)# * 2*math.pi*.0508
        
        velocity = (position - self.previous_pos) / (
                   (self.current_time - self.previous_time) / 1e9)
